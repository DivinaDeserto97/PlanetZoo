...
export function createSlotId(row, column) {
  return `${row}.${column}`;
}
...
export function parseCorridor(value) {
  if (typeof value !== "string") {
    return null;
  }

  const match = value.trim().match(/^L(\d+)\.(\d+)$/i);
...
export function createVerticalCorridorId(row, gapColumn) {
  return `L${2 * row - 1}.${gapColumn}`;
}

export function createHorizontalCorridorId(gapRow, column) {
...
function createLegacyDefaultSlotMap(graph) {
  ...
  const focusIds = new Set(
    nodes.filter((node) => node.focus).map((node) => node.id),
  );

  nodes.forEach((node) => {
    if (node.focus) {
      buckets.focus.push(node);

      return;
    }

    const goesToFocus = graph.edges.some(
      (edge) => edge.from === node.id && focusIds.has(edge.to),
    );

    const comesFromFocus = graph.edges.some(
      (edge) => focusIds.has(edge.from) && edge.to === node.id,
    );

    if (goesToFocus && !comesFromFocus) {
      buckets.input.push(node);
    } else if (comesFromFocus && !goesToFocus) {
      buckets.predator.push(node);
    } else {
      buckets.mixed.push(node);
    }
  });
  ...
  placeBucket(map, used, buckets.input, [1, 2]);

  placeBucket(map, used, buckets.focus, [3]);

  placeBucket(map, used, buckets.mixed, [4]);

  placeBucket(map, used, buckets.predator, [5, 6]);

  return map;
}

function placeBucket(result, used, nodes, columns) {
  nodes.forEach((node, index) => {
    let row = Math.floor(index / columns.length) + 1;

    let column = columns[index % columns.length];

    let slot = createSlotId(row, column);

    while (used.has(slot)) {
      row++;

      slot = createSlotId(row, column);
    }
...
export function getGridSize({ graph, nodeSlots }) {
  let highestRow = 0;

  let highestColumn = 0;

  Object.values(nodeSlots).forEach((slotId) => {
    ...
    highestRow = Math.max(highestRow, slot.row);

    highestColumn = Math.max(highestColumn, slot.column);
  });
  ...
  */
  ...
  const rows = Math.max(GRAPH_GRID.minRows, highestRow + 1);

  const columns = Math.max(GRAPH_GRID.minColumns, highestColumn + 1);
...
export function buildRoutes({
  ...
  */
  ...
  sortedEdges.forEach((edge) => {
    const fromSlot = parseSlot(nodeSlots[edge.from]);
    ...
    const guides = normalizeLocalGuides(
      localEdgeGuides?.[edge.id],
      rows,
      columns,
    );
    ...
    const path = findGuidedCorridorPath({
      fromSlot,
      toSlot,
      rows,
      columns,
    ...
    path.forEach((bereich) => {
      incrementUsage(corridorUsage, bereich);
    });
  });
  ...
  sortedEdges.forEach((edge) => {
    if (routes.has(edge.id)) {
      return;
    }

    const custom = normalizeJsonRoute(edge.route, rows, columns);

    if (!custom) {
      return;
    }

    routes.set(edge.id, custom);

    custom.forEach((step) => {
      incrementUsage(corridorUsage, step.bereich);
    });
  });
  ...
  sortedEdges.forEach((edge) => {
    ...
    const path = findBestCorridorPath({
      fromSlot,
      toSlot,
      rows,
      columns,
  ...
  */
  ...
  sortedEdges.forEach((edge) => {
    ...
    route.forEach((step) => {
      ...
      const jsonLane = Number(step.spur);

      const wanted =
        Number.isInteger(localLane) && localLane > 0
          ? localLane
          : Number.isInteger(jsonLane) && jsonLane > 0
            ? jsonLane
            : null;

      const lane = reserveLane(usedLanes, step.bereich, wanted);
  ...
  return {
    routes,
    laneCounts: getLaneCounts(usedLanes),
  };
}
...
function findBestCorridorPath({ fromSlot, toSlot, rows, columns, usage }) {
  return findPathBetweenCorridors({
    starts: getSlotCorridors(fromSlot, rows, columns),

    targets: new Set(getSlotCorridors(toSlot, rows, columns)),
...
function normalizeLocalGuides(value, rows, columns) {
  if (!value || typeof value !== "object") {
    return null;
  }

  const horizontalGapRow = Number(value.horizontalGapRow);

  const verticalGapColumn = Number(value.verticalGapColumn);

  const result = {};

  if (
    Number.isInteger(horizontalGapRow) &&
    horizontalGapRow >= 1 &&
    horizontalGapRow < rows
  ) {
    result.horizontalGapRow = horizontalGapRow;
  }

  if (
    Number.isInteger(verticalGapColumn) &&
    verticalGapColumn >= 1 &&
    verticalGapColumn < columns
  ) {
    result.verticalGapColumn = verticalGapColumn;
  }
...
function findGuidedCorridorPath({
  ...
  const startCorridors = getSlotCorridors(fromSlot, rows, columns);

  const targetCorridors = new Set(getSlotCorridors(toSlot, rows, columns));

  const horizontal = guides?.horizontalGapRow
    ? Array.from(
        {
          length: columns,
        },
  ...
  if (horizontal.length && vertical.length) {
    ...
    horizontal.forEach((horizontalId) => {
      const neighbors = new Set(
        getCorridorNeighbors(horizontalId, rows, columns),
      );

      vertical.forEach((verticalId) => {
        if (!neighbors.has(verticalId)) {
          return;
        }

        [
          [horizontalId, verticalId],
          [verticalId, horizontalId],
        ].forEach((required) => {
          const candidate = buildPathThroughRequiredCorridors({
            starts: startCorridors,
          ...
          if (
            candidate.length &&
            (!best.length || candidate.length < best.length)
          ) {
            best = candidate;
          }
        });
...
function buildPathThroughRequiredCorridors({
  ...
  for (const requiredId of required) {
    ...
    currentStarts = [requiredId];
  }

  const tail = findPathBetweenCorridors({
    starts: currentStarts,
    targets,
    rows,
    columns,
...
function appendPath(target, part) {
  part.forEach((corridor) => {
    if (target[target.length - 1] !== corridor) {
      target.push(corridor);
    }
  });
}

function findPathBetweenCorridors({ starts, targets, rows, columns, usage }) {
  for (const corridor of starts) {
    if (targets.has(corridor)) {
      return [corridor];
    }
  }
  ...
  starts.forEach((corridor) => {
    const cost = corridorCost(corridor, usage);
    ...
    best.set(corridor, cost);

    previous.set(corridor, null);
  });
  ...
  while (queue.length) {
    ...
    if (current.cost !== best.get(current.corridor)) {
      continue;
    }

    if (targets.has(current.corridor)) {
      return reconstructPath(current.corridor, previous);
    }

    const neighbors = getCorridorNeighbors(current.corridor, rows, columns);

    neighbors.forEach((neighbor) => {
      const nextCost = current.cost + corridorCost(neighbor, usage);

      if (nextCost >= (best.get(neighbor) ?? Infinity)) {
        return;
      }

      best.set(neighbor, nextCost);

      previous.set(neighbor, current.corridor);
...
function corridorCost(corridor, usage) {
  ...
  */
  ...
  return 1 + (usage.get(corridor) ?? 0) * 0.35;
}
...
function getSlotCorridors(slot, rows, columns) {
  ...
  if (slot.column < columns) {
    result.push(createVerticalCorridorId(slot.row, slot.column));
  }

  if (slot.row > 1) {
    result.push(createHorizontalCorridorId(slot.row - 1, slot.column));
  }

  if (slot.row < rows) {
    result.push(createHorizontalCorridorId(slot.row, slot.column));
  }
...
function getCorridorNeighbors(corridorId, rows, columns) {
  ...
  if (corridor.orientation === "vertical") {
    ...
    const gapColumn = corridor.gapColumn;
    ...
    */
    ...
    if (row > 1) {
      result.add(createVerticalCorridorId(row - 1, gapColumn));
    }
    ...
    */
    ...
    if (row > 1) {
      result.add(createHorizontalCorridorId(row - 1, gapColumn));

      result.add(createHorizontalCorridorId(row - 1, gapColumn + 1));
    }
    ...
    */
    ...
    if (row < rows) {
      result.add(createHorizontalCorridorId(row, gapColumn));

      result.add(createHorizontalCorridorId(row, gapColumn + 1));
    }
  } else {
    const gapRow = corridor.gapRow;

    const column = corridor.column;
    ...
    */
    ...
    if (column > 1) {
      result.add(createHorizontalCorridorId(gapRow, column - 1));
    }
    ...
    */
    ...
    if (column > 1) {
      result.add(createVerticalCorridorId(gapRow, column - 1));

      result.add(createVerticalCorridorId(gapRow + 1, column - 1));
    }
    ...
    */
    ...
    if (column < columns) {
      result.add(createVerticalCorridorId(gapRow, column));

      result.add(createVerticalCorridorId(gapRow + 1, column));
    }
  }

  return [...result].filter((id) => isValidCorridor(id, rows, columns));
}

function isValidCorridor(id, rows, columns) {
  const corridor = parseCorridor(id);
  ...
  if (corridor.orientation === "vertical") {
    return (
      corridor.row >= 1 &&
      corridor.row <= rows &&
      corridor.gapColumn >= 1 &&
      corridor.gapColumn < columns
    );
  }

  return (
    corridor.gapRow >= 1 &&
    corridor.gapRow < rows &&
    corridor.column >= 1 &&
    corridor.column <= columns
  );
}
...
function normalizeJsonRoute(route, rows, columns) {
  ...
  const normalized = route
    .map((step) => {
      ...
      return {
        bereich: step.bereich ?? step.korridor ?? "",

        spur: Number.isInteger(Number(step.spur)) ? Number(step.spur) : null,
      };
    })
    .filter((step) => step && isValidCorridor(step.bereich, rows, columns));
...
function reserveLane(usedLanes, corridor, wanted) {
  if (!usedLanes.has(corridor)) {
    usedLanes.set(corridor, new Set());
  }

  const used = usedLanes.get(corridor);
...
function getLaneCounts(usedLanes) {
  const result = {};

  usedLanes.forEach((lanes, corridor) => {
    result[corridor] = Math.max(1, ...lanes);
  });

  return result;
}

function incrementUsage(usage, corridor) {
  usage.set(corridor, (usage.get(corridor) ?? 0) + 1);
}
...
export function buildPixelLayout({ rows, columns, laneCounts }) {
  ...
  for (let gapColumn = 1; gapColumn < columns; gapColumn++) {
    ...
    for (let row = 1; row <= rows; row++) {
      maxLanes = Math.max(
        maxLanes,
        laneCounts[createVerticalCorridorId(row, gapColumn)] ?? 1,
      );
    }

    verticalGapWidths[gapColumn] = getCorridorSize(maxLanes);
  }
  ...
  for (let gapRow = 1; gapRow < rows; gapRow++) {
    ...
    for (let column = 1; column <= columns; column++) {
      maxLanes = Math.max(
        maxLanes,
        laneCounts[createHorizontalCorridorId(gapRow, column)] ?? 1,
      );
    }
  ...
  return {
    ...
    width: x + config.outerPadding,

    height: y + config.outerPadding,
...
function getCorridorSize(laneCount) {
  ...
  return Math.max(
    ...
    config.edgeMargin * 2 + Math.max(0, laneCount - 1) * config.lineGap,
  );
}
...
export function getSlotRect(layout, slotId) {
  const slot = parseSlot(slotId);

  if (!slot || slot.row > layout.rows || slot.column > layout.columns) {
    return null;
  }

  return {
    x: layout.columnLeft[slot.column],
...
export function getCorridorRect(layout, corridorId) {
  const corridor = parseCorridor(corridorId);
  ...
  if (corridor.orientation === "vertical") {
    ...
    return {
      ...
      y: layout.rowTop[corridor.row],

      width: layout.verticalGapWidths[corridor.gapColumn],
  ...
  const top = layout.rowTop[corridor.gapRow] + layout.config.nodeHeight;

  return {
    x: layout.columnLeft[corridor.column],

    y: top,

    width: layout.config.nodeWidth,

    height: layout.horizontalGapHeights[corridor.gapRow],
...
export function getLaneCoordinate(layout, corridorId, lane) {
  ...
  if (rect.orientation === "vertical") {
    return {
      x: rect.x + layout.config.edgeMargin + (lane - 1) * layout.config.lineGap,
  ...
  return {
    ...
    y: rect.y + layout.config.edgeMargin + (lane - 1) * layout.config.lineGap,
  };
}
...
export function findNearestHorizontalGapRow(layout, y) {
  let best = null;

  let bestDistance = Infinity;

  for (let gapRow = 1; gapRow < layout.rows; gapRow++) {
    const rect = getCorridorRect(layout, createHorizontalCorridorId(gapRow, 1));
...
export function findNearestVerticalGapColumn(layout, x) {
  let best = null;

  let bestDistance = Infinity;

  for (let gapColumn = 1; gapColumn < layout.columns; gapColumn++) {
    const rect = getCorridorRect(
      layout,
      createVerticalCorridorId(1, gapColumn),
    );
...
export function findNearestSlot(layout, x, y) {
  let best = null;
