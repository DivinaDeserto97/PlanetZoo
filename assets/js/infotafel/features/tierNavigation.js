...
export function renderTierNavigation(selectedTiere, activeTierId) {
  selectedCount = selectedTiere.length;
  ...
  if (prev) {
    prev.hidden = selectedTiere.length <= 1;
  }

  if (next) {
    next.hidden = selectedTiere.length <= 1;
  }
}
...
export function updateTierNavArrows() {
  ...
  if (!viewport || !track || !prev || !next || selectedCount <= 1) {
    return;
  }
...
export function bindTierNavigationEvents(signal) {
  const track = document.querySelector("[data-tier-nav-track]");
  ...
  window.addEventListener("resize", updateTierNavArrows, {
    signal,
  });
}

export function scrollTierNav(direction) {
  const track = document.querySelector("[data-tier-nav-track]");
