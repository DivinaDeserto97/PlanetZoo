import { clampIndex, getTextEntries, ui, wrapIndex } from "./ui.js";
...
export function renderFacts(tier) {
  ...
  if (text) {
    text.textContent = facts[factIndex]?.inhalt ?? ui("noFacts");
  }
  ...
  if (prev) {
    prev.hidden = facts.length <= 1;
  }

  if (next) {
    next.hidden = facts.length <= 1;
  }
}

export function changeFact(direction, tier) {
  if (!tier) {
    return;
  }
