...
function createAnimalCard(tier, selected, signal) {
...
export function renderHomeTierKarten(tiere, signal) {
  ...
  tiere.forEach((tier) => {
    list.appendChild(createAnimalCard(tier, selected, signal));
  });

  if (resultCount) {
    resultCount.textContent = String(tiere.length);
  }

  if (empty) {
    empty.hidden = tiere.length !== 0;
  }
}
